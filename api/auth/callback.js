const { exchangeCode, connectedEmail } = require('../_lib/google');

/**
 * Google sends the browser here after consent. Not password-gated, because the
 * redirect comes from Google rather than the dashboard — but a code that wasn't
 * issued for this client simply fails the exchange.
 */
...
<p><a href="/dashboard.html" style="color:#00E5FF">Back to the dashboard</a></p>
  ...
  if (oauthError) {
    res.status(400).send(page('Connection cancelled', `Google reported: ${oauthError}`));
    return;
  }
  if (!code) {
    res.status(400).send(page('Something went wrong', 'Google did not send an authorisation code.'));
    return;
  }
  ...
  try {
    ...
    res.status(200).send(page('Connected', `The dashboard is now reading from <strong>${email}</strong>.`));
  } catch (err) {
